      ...
      >
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className={`hidden lg:flex items-center gap-8 backdrop-blur-sm px-8 py-3 rounded-full border transition-all ${isScrolled ? 'bg-white/10 border-white/20' : 'bg-[#2B2118]/5 border-[#2B2118]/10'}`}>
            {menuLinks.map((link) => (
              link.href ? (
                <a key={link.name} href={link.href} className={`font-black text-[15px] uppercase tracking-widest transition-colors no-underline ${isScrolled ? 'text-white hover:text-[#FFF6EA]' : 'text-[#2B2118]/70 hover:text-[#F2641A]'}`}>
                  {link.name}
                </a>
              ) : (
                <button key={link.name} onClick={link.onClick} className={`font-black text-[15px] uppercase tracking-widest transition-colors bg-transparent border-none cursor-pointer ${isScrolled ? 'text-white hover:text-[#FFF6EA]' : 'text-[#2B2118]/70 hover:text-[#F2641A]'}`}>
                  {link.name}
                </button>
              )
            ))}
